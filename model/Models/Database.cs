...
";
		...
		private void LoadTables(SqlCommand cm)
		{
			...
			using (var dr = cm.ExecuteReader())
			{
				LoadTablesBase(dr, false, Tables, DbName);
			}
		...
		private void LoadUserDefinedTypes(SqlCommand cm)
		{
			...
			using (var dr = cm.ExecuteReader())
			{
				LoadUserDefinedTypesBase(dr, UserDefinedTypes, DbName);
			}
		}

		private void LoadUserDefinedTypesBase(SqlDataReader dr,
											  List<UserDefinedType> userDefinedTypes, string dbName)
		{
		...
		private static void LoadTablesBase(SqlDataReader dr, bool areTableTypes, List<Table> tables, string dbName)
		{
			while (dr.Read())
			{
				Table t = new Table((string)dr["TABLE_SCHEMA"], (string)dr["TABLE_NAME"], (DateTime)dr["CREATE_DATE"], (DateTime)dr["MODIFY_DATE"]) { IsType = areTableTypes };
...
";
		...
		private void LoadPartitionFunctions(SqlCommand cm)
		{
			...
			using (var dr = cm.ExecuteReader())
			{
				while (dr.Read())
				{
					...
					pf.DatabaseName = DbName;
					pf.CreateDate = (DateTime)dr["CREATE_DATE"];
					pf.ModifyDate = (DateTime)dr["MODIFY_DATE"];
		...
		private void LoadDbaEvents(SqlCommand cm)
		{
			//get columns
			cm.CommandText = @"
				select * FROM (
			...
			try
			{
				using (var dr = cm.ExecuteReader())
				{
					...
					{
						Table table = null;
						table = FindTableBase(Tables, (string)dr["ObjectName"], (string)dr["SchemaName"]);
						if (table != null)
						{
							table.ModifyHostName = (string)dr["HostName"];
							table.ModifyUserName = (string)dr["UserName"];
						}

						Routine routine = null;
						routine = FindRoutineBase(Routines, (string)dr["ObjectName"], (string)dr["SchemaName"]);
						if (routine != null)
						{
							routine.ModifyHostName = (string)dr["HostName"];
							routine.ModifyUserName = (string)dr["UserName"];
						}
					}
				}
			}
